...
function openEditQtyModal(line, item) {
  ...
  if (el.editQtyItemName) {
    el.editQtyItemName.textContent = `${item?.name || "Deleted item"} (${item?.unit || "pcs"})`;
  }
  if (el.editQtyInput) {
    el.editQtyInput.value = line.quantity;
  }
  if (el.editQtyModal) el.editQtyModal.style.display = "flex";
  if (el.editQtyInput) {
    el.editQtyInput.focus();
    el.editQtyInput.select();
  }
}
...
function saveEditQtyModal() {
  ...
  const newQty = Math.max(1, Number(el.editQtyInput?.value) || 0);
  if (!newQty) {
...
if (el.editQtySaveBtn) {
  el.editQtySaveBtn.addEventListener("click", saveEditQtyModal);
}

if (el.editQtyCancelBtn) {
  el.editQtyCancelBtn.addEventListener("click", closeEditQtyModal);
}
