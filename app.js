...
document.addEventListener("click", (event) => {
  ...
  if (action === "remove-deep-line") {
    ...
    if (confirm("Remove this item from the active purchase order list?")) {
      ...
      saveState();
      syncToSupabase("orders", "delete", { ids: [lineId] });
...
async function applyImportedStockRows(rows) {
  ...
  const newStocks = rows
    ...
    .map((row) => ({
      id: generateUUID(),
      name: row.name.trim(),
      supplierId: findOrCreateSupplierByName(row.supplier, row.email, row.phone, newlyCreatedSuppliers),
  ...
  // Respect foreign keys: clear child rows (orders) before touching stocks,
  // make sure parent rows (suppliers) exist before inserting stocks that reference them.
  ...
  await syncToSupabase("stocks", "replaceAll", { rows: newStocks.map(stockToDb) });
